...
def _generate_representation_and_label_dataset_pytorch(dataset,
                                                       labels,
                                                       representation_functions,
                                                       batch_size,
                                                       num_data_points,
...
"""
    ...
    train_loader = DataLoader(dataset=dataset[indices],
                              batch_size=batch_size,
                              shuffle=False,
                              num_workers=0,
                              pin_memory=True)
    ...
    for i, sample in enumerate(train_loader):
        ...
        for j in range(len(representation_functions)):
            ...
            representation_points[j][i * batch_size:(i + 1) * batch_size, :] = (
                representation_batch[j][0])
    return representation_points, label_points, [np.mean(kl, axis=0) for kl in kl_divergence]
...
@gin.configurable(
    "udr_sklearn_2",
    blacklist=["ground_truth_data", "representation_functions", "random_state", "labels"])
def compute_udr_sklearn_2(ground_truth_data,
                          representation_functions,
                          random_state,
                          batch_size,
                          num_data_points,
  ...
  """
    ...
    if pytorch:
        if labels is not None:
          inferred_model_reps, sampled_labels, kl = _generate_representation_and_label_dataset_pytorch(ground_truth_data,
                                                                                               labels,
                                                                                               representation_functions,
                                                                                               batch_size,
                                                                                               num_data_points,
