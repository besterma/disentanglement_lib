...
def get_datasets():
  ...
  nr_channels_udrvae = h.sweep(
      "vae_quant.UDRVAE.num_channels",
      h.categorical([
          1, 3, 3, 3, 1, 3, 3
      ]))
  ...
  return h.zipit([datasets, nr_channels_udrvae, nr_channels_vae])
...
def get_default_models():
  # udr_vae config.
  model_name = h.fixed("model.name", "udr_beta_tc_vae")
  num_models = h.sweep("UDRVAE.num_models", h.discrete([5]))
  udr_vae = h.zipit([model_name, num_models])
...
class UdrStudy(study.Study):
  ...
  def get_model_config(self, model_num=0):
    ...
    model_config_file = resources.get_file(
        "config/udr_study/model_configs/shared.gin")
  ...
  def get_eval_config_files(self):
    ...
    return list(
        resources.get_files_in_folder(
            "config/udr_study/metric_configs/"))
